...
\citet{1997ApJ...474L..43V}(hereafter \citetalias{1997ApJ...474L..43V}) suggested to use the coherence between different energy bands in order to obtain additional information from the source variability. 
The coherence measure the similarity between two signals and can be computed with the following expression:
\begin{equation}
        C(f) = \frac{|<F_{\rm s}(f)^*F_{\rm h}(f)>|^2 - n^2}{P_{\rm s}(f) P_{\rm h}(f)}
    \label{eq:nowak_coh}
\end{equation}
where $F_{\rm h}(f)$ and $F_{\rm s}(f)$ are Fourier function of the observed time series in hard and soft bands, correspondingly, $P_{\rm s}(f)$ and $P_{\rm h}(f)$ is estimation of their power density spectra (derived, for example, with the Equation~\ref{eq: ps_estimation}),  
...
\end{equation}
Where $Im$ and $Re$ are stated for the imaginary and real part of the complex value correspondingly and $\delta \phi(f)$ if frequency dependent phase lag. 
...
The phase lags observed for different systems had both features, which correlated with the power spectrum and those which had not obvious counterpart in it. 
Therefore, bearing in mind property of the linear evolution of all frequencies of power spectra discussed in previous section, we computed the phase lag spectrum with two approaches - with and without the QPO centroid frequency tracing (see right and left panels of Fig.\ref{fig:phase_lag}, correspondingly).
