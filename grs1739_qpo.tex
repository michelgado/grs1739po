...
\begin{abstract}
We detected type-C LF QPO at 0.3..0.7 Hz in \nustar\, and \swiftx\, observations of black hole candidate \grs during the hard-intermediate state of its 2014 outburst. We traced the evolution of spectro-timing properties of the source during \nustar\ observation. 
As QPO frequency grows, source spectrum became softer, with increasing power-law index and decreasing cut-off energy.

We performed extended analysis of rapid X-ray variability in terms of power spectrum, cospectrum, coherence and phase-lags. 
In power spectrum there are prominent QPO and its second harmonic. 
Flux in soft and hard X-ray bands are coherent, however coherence drops with the energy bands separation. 
Phase-lags are generally positive (hard) in the 0.1--3~Hz frequency range, and negative below 0.1~Hz.
%Coherence between neighboring energy channels seems to be unity, while separated channels shows lower value. Phase-lags are found to be...
Measurements of the inner disk radius obtained with the spectral models accounting for the metric around black and hole and QPO frequency together with the relativistic precession model points towards very massive black hole.
%Using inner disk radius estimated from average spectrum of the source and the QPO frequency we show that there are some tensions between QPO Lense-Thirring precession model and relativistic spectral model.
\end{abstract}
...
This works both on a long timescales - i.e. days and weeks - when one speaks about state changes through outbursts of transients sources \citep[see e.g.][]{2005Ap&SS.300..107H, heil15}, and on short - all the way down to milliseconds - when the subject under consideration are a quasi-periodic oscillations (QPOs) and broad band stochastic noise. 
...
It is strongly suggested from the observations that this geometrically thick hot flow is irresponsible for production of strong variability. 
...
One of the long-standing questions in X-ray timing is the origin of so-called type-C low-frequency QPOs \citep[see][for LFQPO classification]{casella05}, which are often found in X-ray binaries harboring both neutron stars and black-holes \citep{wijnands99} during initial rise and transition to disk dominated state - i.e. in low-hard state (LHS) and in hard intermediate state (HIMS), according to standard scheme \citep{grebenev97, tanaka96, remillard06, belloni10}. 
These QPOs are widespread and easy to detect and study, since they occur at moderate frequencies of 0.1..10 Hz and prominent, having $rms\approx10\%$. 
...
Recent advances in simulations of the reflected emission \citep{ross05,garcia14}, arising due to the scattering and absorption of the hard photons in the cold accretion disk, led to the possibility to study geometry of the disk. %bright systems. 
...
Now, adding information from X-ray timing one can, in principle, constrain the location of a component, responsible for the variability - which is though to be corona or jet base.
...
After this outburst the source remain active with repetitive mini-outbursts \citep{mereminskiy16,yan17}.
...
We used public observations of \swiftx\, (target ID: 33203) performed regularly over the peak and decline of the outburst.  
...
\nustar\, observed \grs\ (ObsID: 80002018002) for nearly 30 ks of net exposure right after the hard X-ray peak (see Fig.~\ref{fig:batlc}). 
...
To get better view on the evolution of continuum emission we fitted all individual interval spectra using \texttt{XSPEC} package \citep{arnaud96} with \texttt{xillver} model \citep{garcia13}. 
...
Spectra from two \nustar\, modules of each interval were fitted simultaneously with \texttt{phabs*const*xillver} model. 
...
Flux is steadily increasing and at the end of the observation unabsorbed luminosity in 3--60 keV band reaches $7\times10^{37}$ erg s$^{-1}$ for 8 kpc distance (assuming that the source is located in the Galaxy bulge). 

\subsubsection{Constrains on movement of the inner parts of accretion disk}
Spectra of single intervals have not enough statistics to constrain change of Fe-line profile and, hence, to determine is the inner boundary of disk is moving during observation. 
...
We extracted \swiftx\, spectrum using only zero-grade events, and grouped it to has at least 30 counts per bin and added 3\% systematic error. 
Similar grouping were applied to \nustar\, data. 
We used latest available version of {\it relxilllp} package (v1.0.2), the model that describes reflection of emission produced by point source located on the rotation axis above the Kerr black hole from the relativistic accretion disk. 
...
Total flux in 0.1--100 keV band is about 10$^{-8}$ erg s$^{-1}$ cm$^{-2}$ which translates to a luminosity of 7.7$\times$10$^{37}$ erg s$^{-1}$ for the 8 kpc distance. 
...
Therefore one can put a rough lower estimate on the black-hole mass as 6.5 $M_{\odot}$, which is reasonable.
...
Power spectrum of each of separate interval of {\it NuSTAR} observations has a form of a white noise plateau ($P(f)\propto const$) on the low frequencies, transforming on the frequency $\approx0.1$~Hz in to the power law with the slope $\rho\approx-1.6$..$-2.0$. 
Also a prominent QPO on the frequencies 0.3--0.7~Hz and its second harmonic are present. 
Typical power spectrum of a single interval is shown in Fig.~\ref{fig:qpo}.
Poisson noise dominates intrinsic source variability on the frequencies over $\approx1$~Hz, preventing analysis of any high-frequency features.
...
{\it NuSTAR} detectors are subject to non-paralyzing dead time with characteristic timescale $\tau \approx 2.5$~$\mu$s \citep{2015ApJ...800..109B}.
...
Since we used relatively large time bining (10 ms) to extract lightcurves from {\it NuSTAR} data, we assumed that the only effect from the dead-time is lowering of the constant Poisson noise level on the $(1 - 2\nu \tau_{\rm d})$ factor, where $\nu$ is total count rate for detector and $\tau_{\rm d}$ is a dead time \citep{1994A&A...287...73V, 1995ApJ...449..930Z}. 
As was mentioned, dead-time is not constant along energy band, therefore we decided to measure modified Poisson level for each extracted data-set.
...
We found that the QPO frequency had clearly evolved with time see Table~\ref{tab:ps} and Figure~\ref{fig:qpo}.
...
Some models suggest that type-C QPO arise due to the Lense-Thirring precession of the accretion flow inner parts \citep{1998ApJ...492L..59S, 2006ApJ...642..420S, 2009MNRAS.397L.101I}.
...
In some intervals QPO subharmonics, centered approximately at the 1/2 of the QPO centroid frequency, is clearly observed in the cospectra (see examples on Fig.~\ref{fig:subharmonic}, red crosses) (namely I, II, IV, V, VI sets).
...
\begin{equation}
    ...
    \label{eq:nowak_coh}
\end{equation}
where $F_{\rm h}(f)$ and $F_{\rm h}(f)$ are Fourier function (corrected for the Poisson noise components) of the observed time series in hard and soft bands correspondingly, 
...
Therefore, in order to improve significance of the coherence measurements, and bearing in mind aforementioned property of the power spectrum, we stacked all 13 separate intervals of the observation, scaling their frequencies to preserve QPO position. 
...
On Fig.~\ref{fig:coherence} presented measured coherence between hard and soft bands on the frequencies up to $\sim3$~Hz. 
...
As it was discussed in the previous section,  following \citet{1997ApJ...474L..43V} we estimated phase lag as an angle of mean product of the light curves Fourier harmonics from one energy bands to the conjugated Fourier harmonics of the second energy band. 
\begin{equation}
