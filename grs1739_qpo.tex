...
QPO is clearly detected in observations 3 to 9, with frequency varying from 0.37 Hz (during simultaneous observation with \nustar, see Fig.~\ref{fig:nust_lc}) up to 5.1 Hz (see Tab.~\ref{tab:xrtqpo}).

We used shape of power spectrum in order to classify QPOs as belonging to type C or B \citep{casella05}. Except for the observation 03, there are no strong low-frequency noise components present in power spectra. Observed features are more like red-noise, which is typical for type-B QPOs in soft-intermediate state, although we should note that due to shortness of observations low-frequency parts of power spectra are subject to strong uncertainties.

Observed frequencies of these QPOs ranges from $\sim$2~Hz to 5~Hz, similar to what had been observed in GX~339-4 \citep{motta11}. However, all observations presented here were taken during outburst rise phase, while in \citet{motta11} type-B QPOs with frequencies below 3~Hz were present only in late phases  of outburst.
In order to reinforce classification we calculated $rms$ for each detected QPO, along with total $rms$ over 0.01--20 Hz band. Obtained $rms_{total}$ are significantly lower for observations 04..09 than for 03. We, therefore, conclude that in observations 04..09 type-B QPOs were observed.

As an additional cross-check we calculated power color PC2, defined in \citet{heil15}, for all observations. Its value is $1.1_{-0.3}^{+0.7}$ for the 03 observation and in range of 0.1--0.2 for all others. This result confirm that the source was in soft-intermediate state during \swiftx\ snapshots 04..09 and observed QPOs belongs to type-B. Therefore, four days after a \nustar\ observation took a place source already had transitioned from HIMS to SIMS. 
...
\hline\hline
Segment & $f_{QPO}$, Hz & QPO $rms$, \% & Total $rms$, \% & Type\\
\hline
03  &  $0.37\pm0.01$            & $9\pm2$       & $28\pm3$        & C\\
04  &  $2.17\pm0.03$            & $8_{-2}^{+1}$ & $14_{-1}^{+2}$  & B\\
05  &  $1.67_{-0.04}^{+0.03}$   & $8\pm1$       & $17_{-3}^{+6}$  & B\\ 
06  &  $5.05\pm0.09$            & $4_{-1}^{+2}$ & $11_{-1}^{+2}$  & B\\
07  &  $2.52_{-0.08}^{+0.07}$   & $7_{-2}^{+1}$ & $15_{-2}^{+3}$  & B\\ 
08  &  $5.10_{-0.14}^{+0.16}$   & $5\pm1$       & $12_{-2}^{+3}$  & B\\
09  &  $2.17_{-0.04}^{+0.02}$   & $7_{-1}^{+2}$ & $17_{-3}^{+4}$  & B\\
\hline
\end{tabular}
\end{table}
