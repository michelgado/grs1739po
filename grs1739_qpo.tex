...
Outburst profile in hard X-rays (15--50~keV) featured fast rise with tenfold intensity increase over ten days, nearly flat-top peak ($\tau_{0}$+10..+15 days) followed by abrupt flux decrease by 30\% over two days.
...
Observed features are more like red-noise, which is typical for type-B QPOs in soft-intermediate state, although we should note that due to shortness of observations low-frequency parts of power spectra are subject to strong uncertainties.

Observed frequencies of these QPOs ranges from $\sim$2~Hz to 5~Hz, {\bf similar to what had been observed in GX~339-4} \citep{motta11}. 
{\bf However, all observations presented here were taken during outburst rise phase, while in \citet{motta11} type-B QPOs with frequencies below 3~Hz were present only in the late phases of outburst.}
...
We, therefore, conclude that in observations 04..09 type-B QPOs were observed.

As an additional cross-check we calculated power color PC2, defined in \citet{heil15}, for all observations. Its value is $1.1_{-0.3}^{+0.7}$ for the 03 observation and in range of 0.1--0.2 for all others. This result confirm that the source was in soft-intermediate state during \swiftx\ snapshots 04..09 and observed QPOs belongs to type-B. 
Therefore, we can conclude that four days after the \nustar\ observation took place, the source already had transitioned from HIMS to SIMS. 
